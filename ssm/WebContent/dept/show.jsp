...
<script>
   function del(deptId){
     if(confirm("是否删除此数据?")){
        window.location.href="<%=path%>/dept/del.do?id="+deptId;
     }
   }
</script>
<head>
  ...
  <tr>
    ...
    <table width="100%" border="0" cellspacing="0" cellpadding="0" id="main-tab">
      <tr>
        ...
        <th align="center" valign="middle" class="borderright">部门经理</th>
      ...
      <tr class="bggray" onMouseOut="this.style.backgroundColor='#f9f9f9'" onMouseOver="this.style.backgroundColor='#edf5ff'">
        <td align="center" valign="middle" class="borderright borderbottom">${record.name}</td>
        <td align="center" valign="middle" class="borderright borderbottom">${record.manager}</td>
        <td align="center" valign="middle" class="borderbottom">
           <a href="<%=path %>/dept/${record.id}/load" target="mainFrame" onFocus="this.blur()" class="add">编辑</a><span class="gray">&nbsp;|&nbsp;</span>
           <a href="javascript:del(${record.id});" target="mainFrame" onFocus="this.blur()" class="add">删除</a>
        </td>
      </tr>
     </c:forEach>
    </table></td>
